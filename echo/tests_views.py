...
from echo.models import SMSCampaign, Balance, MailCampaign
from echo.utils import count_pages
from echo.cron_sender import restart_sms_batch, restart_mail_batch
...
class CampaignTestCase(unittest.TestCase):
    ...
    def test_restart_sms_batch(self):
        call_command('loaddata', 'echo_campaign.yaml', database='umbrella')
        restart_sms_batch()
        campaign = SMSCampaign.objects.using('umbrella').get(service='56eb6d04b37b3379b531b102')
        ...
        self.assertLess(balance.sms_count, 100)

    @override_settings(IKWEN_SERVICE_ID='56eb6d04b37b3379b531b102',
                       EMAIL_BACKEND='django.core.mail.backends.filebased.EmailBackend',
                       EMAIL_FILE_PATH='test_emails/echo/',
                       UNIT_TESTING=True)
    def test_restart_mail_batch(self):
        call_command('loaddata', 'echo_campaign.yaml', database='umbrella')
        restart_mail_batch()
        campaign = MailCampaign.objects.using('umbrella').get(service='56eb6d04b37b3379b531b102')
        self.assertGreater(campaign.progress, 0)
        balance = Balance.objects.using(WALLETS_DB_ALIAS).get(service_id='56eb6d04b37b3379b531b102')
        self.assertLess(balance.mail_count, 50)
    ...
    def test_confirm_bundle_payment_with_momo_payment(self):
        ...
        balance = Balance.objects.using(WALLETS_DB_ALIAS).get(service_id='56eb6d04b37b3379b531b102')
        self.assertEqual(balance.mail_count, 10050)
